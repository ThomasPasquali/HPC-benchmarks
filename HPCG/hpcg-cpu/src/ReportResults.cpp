...
void ReportResults(const SparseMatrix & A, int numberOfMgLevels, int numberOfCgSets, int refMaxIters,int optMaxIters, double times[],
  ...
  double t6max = 0.0;
  double t6avg = 0.0;
  MPI_Allreduce(&t6, &t6min, 1, MPI_DOUBLE, MPI_MIN, MPI_COMM_WORLD);
  MPI_Allreduce(&t6, &t6max, 1, MPI_DOUBLE, MPI_MAX, MPI_COMM_WORLD);
  MPI_Allreduce(&t6, &t6avg, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
  t6avg = t6avg/((double) A.geom->size);
#endif
  ...
  if (A.geom->rank==0) { // Only PE 0 needs to compute and report timing results
    ...
    doc.get("Sparse Operations Overheads")->add("Min Halo exchange time",t6min);
    doc.get("Sparse Operations Overheads")->add("Max Halo exchange time",t6max);
    doc.get("Sparse Operations Overheads")->add("Avg Halo exchange time",t6avg);
