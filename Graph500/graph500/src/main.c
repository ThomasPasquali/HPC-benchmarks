...
int main(int argc, char** argv) {
	...
	if (!getenv("SKIP_BFS")) {
		...
		for (bfs_root_idx = 0; bfs_root_idx < NUM_BFS_ROOTS; ++bfs_root_idx) {
			...
			int64_t edge_cut_visit_count=0;
			get_edge_on_cut_count_for_teps(&edge_cut_visit_count);
			cut_edge_counts[bfs_root_idx] = (double)edge_cut_visit_count;
			#ifdef VERBOSE_PRINTS
	...
	for (bfs_root_idx = 0; bfs_root_idx < NUM_BFS_ROOTS; ++bfs_root_idx) {
		...
		cut_edge_counts[bfs_root_idx] = (double)edge_cut_visit_count;
		#ifdef VERBOSE_PRINTS
			if (rank == 0) fprintf(stderr, "Cut edges: %d\n", edge_cut_visit_count);
			if (rank == 0) fprintf(stderr, "Cut TEPS in BFS %d is %g\n", bfs_root_idx, edge_cut_visit_count / bfs_times[bfs_root_idx]);
		#endif
